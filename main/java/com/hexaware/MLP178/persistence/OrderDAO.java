 ...
 */
  ...
  @SqlUpdate("INSERT INTO ORDERS(ORD_ID, CUS_ID, VEN_ID, MEN_ID1, MEN_ID2, MEN_ID3,"
      + "ORD_STATUS, ORD_COMMENTS, ORD_TOTAL_AMT, ORD_DATE, ORD_QTY, WAL_TYPE) "
      + " VALUES(:orderId, :customerId, :vendorId, :menuId1, :menuId2, :menuId3, :orderStatus,"
...
/**
  * @param custId for accepting customer id.
  * @return the selected record.
  */
  @SqlQuery("SELECT * FROM CUSTOMER WHERE CUS_ID=:custId")
  int checkGender1(@Bind("custId") int custId);

  /**
  * @param gendr for accepting customer id.
  * @return the selected record.
  */
  @SqlQuery("SELECT * FROM CUSTOMER WHERE CUS_GENDER=:gendr")
  int getGender(@Bind("gendr") int gendr);
}
